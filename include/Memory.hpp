...
namespace wfe {
    ...
    void* malloc(size_t size, MemoryUsage memoryUsage = MEMORY_USAGE_OTHER);
    ...
    /// @param size The size of the memory block.
    /// @return A pointer to the resulting memory block, or a nullptr if the block couldn't be allocated.
    void* calloc(size_t nmemb, size_t size, MemoryUsage memoryUsage = MEMORY_USAGE_OTHER);
    ...
    void* realloc(void* ptr, size_t oldSize, size_t newSize, MemoryUsage memoryUsage = MEMORY_USAGE_OTHER);
    ...
    /// @return An integer representing the relationship between the two memory blocks: < 0 if the first memory block is smaller than the second, 0 of the two memory blocks are equal and > 0 if the first memory block if bugger than the second.
    ...
    /// @return An integer representing the relationship between the two strings: < 0 if the first string is smaller than the second, 0 of the two strings are equal and > 0 if the first string if bugger than the second.
    ...
    /// @return An integer representing the relationship between the two string sections: < 0 if the first string section is smaller than the second, 0 of the two string sections are equal and > 0 if the first string section if bugger than the second.
